...
extension String {
    ...
    func replace(target: String, withString: String) -> String {
        replacingOccurrences(of: target, with: withString)
    }
    ...
    func subStringInRange(from: Int, to: Int) -> String {
        guard from >= 0, to >= from, from < count else {
            return ""
        }
    ...
    func getSubStringSeparatedByFirst(_ char: String) -> String {
        components(separatedBy: char).first ?? ""
    }

    /// Splits the string into components using the supplied separator.
    func componentsSeparatedByChar(_ char: String) -> [String] {
    ...
    func isValidURL() -> Bool {
        URL(string: self)?.scheme?.lowercased().hasPrefix("http") == true
    }
    ...
    func startsWithText(_ text: String) -> Bool {
        hasPrefix(text)
    }
    ...
    func getLimitedLength(_ maxLength: Int) -> String {
