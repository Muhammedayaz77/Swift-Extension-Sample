...
extension UICollectionView {
    ...
    func getItemCount(section: Int) -> Int {
        return section < numberOfSections ? numberOfItems(inSection: section) : 0
    }

    /// Returns whether an index path exists.
    func hasItem(_ indexPath: IndexPath) -> Bool {
        return indexPath.section < numberOfSections
            && indexPath.item < numberOfItems(inSection: indexPath.section)
    }

    /// Scrolls to the top-left.
    func scrollCollectionToTop(animated: Bool = true) {
        setContentOffset(
    ...
    func scrollCollectionToBottom(animated: Bool = true) {
        let y = max(
    ...
    func selectItemSafely(_ indexPath: IndexPath, animated: Bool = true) {
        if hasItem(indexPath) {
            selectItem(at: indexPath, animated: animated, scrollPosition: [])
        }
    }

    /// Sets collection background color.
    func setCollectionBackgroundColor(_ color: UIColor) {
        backgroundColor = color
    }

    /// Enables scrolling.
    func enableCollectionScrolling() {
        isScrollEnabled = true
    }

    /// Disables scrolling.
    func disableCollectionScrolling() {
        isScrollEnabled = false
    }

    /// Hides vertical indicator.
    func hideCollectionVerticalIndicator() {
        showsVerticalScrollIndicator = false
    }

    /// Hides horizontal indicator.
    func hideCollectionHorizontalIndicator() {
        showsHorizontalScrollIndicator = false
    }

    /// Enables paging.
    func enableCollectionPaging() {
        isPagingEnabled = true
    }

    /// Disables paging.
    func disableCollectionPaging() {
