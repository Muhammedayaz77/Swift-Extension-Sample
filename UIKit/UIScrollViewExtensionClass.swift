...
extension UIScrollView {
    ...
    func scrollToLeft(animated: Bool = true) {
        setContentOffset(
            CGPoint(x: -adjustedContentInset.left, y: contentOffset.y),
            animated: animated
        )
    }
    ...
    func getHorizontalOffset() -> CGFloat {
        contentOffset.x
    }
    ...
    func setIndicatorInsets(_ insets: UIEdgeInsets) {
        scrollIndicatorInsets = insets
    }

    /// Sets content inset.
    func setScrollContentInsets(_ insets: UIEdgeInsets) {
        contentInset = insets
    }
}
